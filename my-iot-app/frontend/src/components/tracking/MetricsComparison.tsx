...
import { GaitMetrics, ComparisonMetric } from '@/types/tracking.types';

interface MetricsComparisonProps {
  currentMetrics: GaitMetrics | null;
}

const MetricsComparison: React.FC<MetricsComparisonProps> = ({ currentMetrics }) => {
    ...
    <div className="card">
      ...
      <div className="space-y-4">
        {comparisonMetrics.map((metric) => (
          <div key={metric.name} className="border-b border-gray-200 pb-3">
              ...
              <div className="flex-1">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.min((metric.left / Math.max(metric.left, metric.right)) * 100, 100)}%` }}
                  />
                </div>
              </div>
              <div className="flex-1">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.min((metric.right / Math.max(metric.left, metric.right)) * 100, 100)}%` }}
                  />
                </div>
              </div>
            </div>
